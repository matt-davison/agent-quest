...
function parseWorldArg() {
  ...
  if (!worldArg) {
    ...
    console.error(
      "Usage: node world-state.js --world=<world-id> <domain> <command> [args]",
    );
...
function loadDirectory(dirPath, excludeFiles = ["_meta.yaml"]) {
  ...
  const files = fs
    .readdirSync(dirPath)
    .filter((f) => f.endsWith(".yaml") && !excludeFiles.includes(f));
...
function loadGraph() {
  ...
  return {
    connections,
    travel_speeds: meta.travel_speeds || {
      walking: 1,
      running: 2,
      mounted: 3,
      flying: 5,
...
function isAreaUnlockedForCharacter(areaId, player, character) {
  ...
  if (globalState.unlocked_areas) {
    const globalUnlock = globalState.unlocked_areas.find(
      (a) => a.area_id === areaId,
    );
    if (globalUnlock)
      return { unlocked: true, source: "global", data: globalUnlock };
  }

  // Check character-specific unlock
  if (charOverrides && charOverrides.unlocked_areas) {
    const charUnlock = charOverrides.unlocked_areas.find(
      (a) => a.area_id === areaId,
    );
    if (charUnlock)
      return { unlocked: true, source: "character", data: charUnlock };
  }
...
function getNpcLocationForCharacter(npcId, player, character) {
  ...
  if (charOverrides && charOverrides.npc_overrides) {
    const override = charOverrides.npc_overrides.find(
      (o) =>
        o.npc_id === npcId &&
        (o.override_type === "location" || o.override_type === "state"),
    );
    if (override) {
...
function timeGet() {
  ...
  console.log(
    `Time: ${String(t.hour).padStart(2, "0")}:${String(t.minute).padStart(
      2,
      "0",
    )}`,
  );
...
function timeAdvance(hours) {
  ...
  console.log(
    `New time: Day ${t.day}, Month ${t.month}, Year ${t.year} - ${String(
      t.hour,
    ).padStart(2, "0")}:00 (${t.period})`,
  );
}
...
function weatherGet(region) {
  ...
  if (!weather) {
    ...
    console.log(
      `Available regions: ${Object.keys(state.weather.by_region).join(", ")}`,
    );
    process.exit(1);
  }
...
function weatherRoll(region) {
  ...
  const newTempIndex = Math.max(
    0,
    Math.min(temps.length - 1, tempIndex + shift),
  );
...
function npcLocation(npcId) {
  ...
  const pattern = schedule.patterns.find(
    (p) => p.times && p.times.includes(currentPeriod),
  );
...
function npcAt(location) {
  ...
  for (const [npcId, schedule] of Object.entries(schedules)) {
    ...
    if (state.npc_location_overrides && state.npc_location_overrides[npcId]) {
      if (state.npc_location_overrides[npcId].location === location) {
        npcsPresent.push({
          id: npcId,
          activity: state.npc_location_overrides[npcId].reason,
        });
      }
      continue;
    }
    ...
    if (pattern && pattern.location && pattern.location.startsWith(location)) {
      ...
      schedule.default_location.startsWith(location)
    ) {
  ...
  for (const [npcId, npc] of Object.entries(npcs)) {
    if (
      ...
      npc.default_location.startsWith(location)
    ) {
...
function npcAvailable(npcId) {
  ...
  if (typeof pattern.interruptible === "string") {
    ...
    console.log(
      `${npcId} is conditionally available: ${pattern.interruptible}`,
    );
    return;
  }
...
function calculateTravelTime(distance, speed = "walking", graph) {
  const speeds = graph.travel_speeds || {
    walking: 1,
    running: 2,
    mounted: 3,
    flying: 5,
...
function travelRoute(from, to, speed = "walking") {
  ...
  const encounterChance =
    normalizedDanger === "high"
      ? "high (~70%)"
      : normalizedDanger === "moderate"
      ? "moderate (~50%)"
      : normalizedDanger === "low"
      ? "low (~30%)"
      : "none";
...
function travelStart(playerId, from, to, speed = "walking") {
  ...
  if (state.travelers_in_transit[playerId]) {
    ...
    console.log(
      `Current journey: ${state.travelers_in_transit[playerId].from} → ${state.travelers_in_transit[playerId].to}`,
    );
    process.exit(1);
  }
...
function travelProgress(playerId, hours) {
  ...
  travel.hours_traveled = Math.min(
    travel.hours_traveled + h,
    travel.duration_hours,
  );
  ...
  if (travel.encounters) {
    for (const enc of travel.encounters) {
      if (
        !enc.resolved &&
        enc.hour > prevHours &&
        enc.hour <= travel.hours_traveled
      ) {
  ...
  console.log(
    `Progress: ${travel.hours_traveled}/${travel.duration_hours} hours`,
  );
  console.log(
    `World time advanced to: ${state.time.hour}:00 (${state.time.period})`,
  );
...
function travelStatus(playerId) {
  ...
  if (!playerId) {
    ...
    if (
      !state.travelers_in_transit ||
      Object.keys(state.travelers_in_transit).length === 0
    ) {
    ...
    for (const [id, travel] of Object.entries(state.travelers_in_transit)) {
      const pct = Math.round(
        (travel.hours_traveled / travel.duration_hours) * 100,
      );
  ...
  console.log(
    `  Progress: ${travel.hours_traveled}/${travel.duration_hours} hours (${pct}%)`,
  );
...
function travelComplete(playerId) {
  ...
  const unresolved =
    travel.encounters?.filter(
      (e) => !e.resolved && e.hour <= travel.hours_traveled,
    ) || [];
  if (unresolved.length > 0) {
    console.error(
      `Cannot complete travel: ${unresolved.length} unresolved encounter(s)`,
    );
  ...
  if (travel.hours_traveled < travel.duration_hours) {
    console.error(
      `Journey not complete: ${
        travel.duration_hours - travel.hours_traveled
      } hours remaining`,
    );
    console.log(
      `Use 'travel progress ${playerId} <hours>' to continue traveling.`,
    );
    process.exit(1);
  }
...
function travelEncounterResolve(playerId, encounterHour) {
  ...
  const remaining = travel.encounters.filter(
    (e) => !e.resolved && e.hour <= travel.hours_traveled,
  );
...
function eventsRecent(days) {
  ...
  events.slice(0, 10).forEach((event) => {
    ...
    console.log(
      `    Type: ${event.type}, Regions: ${event.regions?.join(", ") || "all"}`,
    );
  });
}
...
function stateOverrides() {
  if (!PLAYER_ID || !CHARACTER_ID) {
    console.error(
      "Error: --player and --character are required for state overrides",
    );
    console.error(
      "Usage: node world-state.js --world=alpha --player=<github> --character=<name> state overrides",
    );
    process.exit(1);
  }
  ...
  if (!charOverrides) {
    console.log(
      `No character-specific overrides for ${PLAYER_ID}/${CHARACTER_ID}`,
    );
...
function areaUnlocked(areaId) {
  if (!areaId) {
    console.error(
      "Usage: node world-state.js --world=<world> area unlocked <area-id> [--player=<github> --character=<name>]",
    );
    process.exit(1);
  }
  ...
  if (PLAYER_ID && CHARACTER_ID) {
    ...
    if (result.unlocked) {
      ...
      if (result.data.unlock_source)
        console.log(`Unlock source: ${result.data.unlock_source}`);
      if (result.data.unlocked_date)
        console.log(`Unlocked: ${result.data.unlocked_date}`);
    } else {
      console.log(`false`);
    ...
    const globalState = loadYaml(STATE_PATH);
    if (globalState.unlocked_areas) {
      const unlock = globalState.unlocked_areas.find(
        (a) => a.area_id === areaId,
      );
      if (unlock) {
        console.log(`true`);
    ...
    console.log(
      `(Use --player and --character to check character-specific unlocks)`,
    );
  }
}
...
function overrideSet(type, key, value) {
  if (!PLAYER_ID || !CHARACTER_ID) {
    console.error(
      "Error: --player and --character are required for override set",
    );
    console.error(
      "Usage: node world-state.js --world=alpha --player=<github> --character=<name> override set <type> <key> <value>",
    );
    process.exit(1);
  }
  ...
  switch (type) {
    ...
    case "area_unlock":
      ...
      charOverrides.unlocked_areas = charOverrides.unlocked_areas.filter(
        (a) => a.area_id !== key,
      );
    ...
    case "npc_location":
      ...
      console.log(
        `Set NPC '${key}' location to '${value}' for ${CHARACTER_ID}`,
      );
      break;
  ...
  console.log(
    `Saved to ${getCharacterWorldStatePath(PLAYER_ID, CHARACTER_ID)}`,
  );
}
...
function overrideClear(type, key) {
  if (!PLAYER_ID || !CHARACTER_ID) {
    console.error(
      "Error: --player and --character are required for override clear",
    );
    console.error(
      "Usage: node world-state.js --world=alpha --player=<github> --character=<name> override clear <type> <key>",
    );
    process.exit(1);
  }
  ...
  if (!type || !key) {
    ...
    console.error(
      "Types: flag, area_unlock, npc_location, npc_state, npc (clears all NPC overrides for that NPC)",
    );
    process.exit(1);
  }
  ...
  if (!charOverrides) {
    console.log(
      `No character overrides exist for ${PLAYER_ID}/${CHARACTER_ID}`,
    );
    return;
  }
  ...
  switch (type) {
    ...
    default:
      console.error(`Unknown override type: ${type}`);
      console.error(
        "Valid types: flag, area_unlock, npc_location, npc_state, npc",
      );
      process.exit(1);
  }
...
function loadLocationCoordinates(locationId) {
  const locData = loadLocationData(locationId);
  if (!locData || !locData.coordinates) {
    return null;
  }
  return {
    x: locData.coordinates.x || 0,
    y: locData.coordinates.y || 0,
    z: locData.coordinates.z || 0,
  };
}

function getAllLocationCoordinates() {
  const locations = [];
  if (fs.existsSync(LOCATIONS_PATH)) {
    const entries = fs.readdirSync(LOCATIONS_PATH, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isDirectory()) continue;
      const locData = loadLocationData(entry.name);
      if (locData && locData.coordinates) {
        locations.push({
          id: locData.id || entry.name,
          name: locData.name || entry.name,
          coordinates: {
            x: locData.coordinates.x || 0,
            y: locData.coordinates.y || 0,
            z: locData.coordinates.z || 0,
...
function calculate3DEuclideanDistance(coord1, coord2) {
  const dx = coord2.x - coord1.x;
  const dy = coord2.y - coord1.y;
...
function distanceCalculate(from, to, method = "euclidean") {
  const coord1 = loadLocationCoordinates(from);
  const coord2 = loadLocationCoordinates(to);

  if (!coord1) {
    console.error(`Location not found or has no coordinates: ${from}`);
    process.exit(1);
  }
  if (!coord2) {
    console.error(`Location not found or has no coordinates: ${to}`);
    process.exit(1);
  }
  ...
  switch (method) {
    case "euclidean":
      distance = calculateEuclideanDistance(coord1, coord2);
      break;
    case "manhattan":
      distance = calculateManhattanDistance(coord1, coord2);
      break;
    case "3d":
      distance = calculate3DEuclideanDistance(coord1, coord2);
      break;
    default:
      console.error(`Unknown method: ${method}. Use: euclidean, manhattan, 3d`);
      process.exit(1);
  }
  ...
  console.log(`From: ${from} (${coord1.x}, ${coord1.y}, ${coord1.z})`);
  console.log(`To: ${to} (${coord2.x}, ${coord2.y}, ${coord2.z})`);
  console.log(`Method: ${method}`);
  console.log(`Distance: ${roundedDistance} leagues`);

  // Also show travel comparison if connection exists
  const graph = loadGraph();
  const conn = findConnection(graph, from, to);
  if (conn) {
    console.log(`Travel distance (from connection): ${conn.distance} leagues`);
  }
}

function distanceList() {
  const locations = getAllLocationCoordinates();
  ...
  for (const loc of locations) {
    console.log(
      `${loc.id}: (${loc.coordinates.x}, ${loc.coordinates.y}, ${loc.coordinates.z}) - ${loc.name}`,
    );
  }
}

function distanceMatrix(method = "euclidean") {
  const locations = getAllLocationCoordinates();
  if (locations.length === 0) {
    console.log("No locations with coordinates found.");
    return;
  ...
  const calcFn =
    method === "manhattan"
      ...
      : method === "3d"
      ? calculate3DEuclideanDistance
      : calculateEuclideanDistance;

  console.log(`Distance matrix (${method}):`);
  console.log("---");
  ...
  for (const loc1 of locations) {
    ...
    for (const loc2 of locations) {
      if (loc1.id === loc2.id) {
        ...
        const dist = calcFn(loc1.coordinates, loc2.coordinates);
        matrix[loc1.id][loc2.id] = Math.round(dist * 100) / 100;
      }
    }
  }

  // Output as YAML for easy reading
  console.log(yaml.stringify(matrix));
}
...
Examples:
  ...
  case "distance":
    switch (command) {
      case "calc":
      case "calculate":
        distanceCalculate(args[0], args[1], args[2]);
        break;
      case "list":
        distanceList();
        break;
      ...
      default:
        console.log(`Usage:
  distance calc <from> <to> [method]   Calculate distance between locations
  distance list                        List all locations with coordinates
  distance matrix [method]             Show distance matrix for all locations

Methods:
  euclidean   2D straight-line distance (default): sqrt((x2-x1)^2 + (y2-y1)^2)
  manhattan   Grid-based distance: |x2-x1| + |y2-y1|
  3d          3D euclidean including elevation: sqrt((x2-x1)^2 + (y2-y1)^2 + (z2-z1)^2)

Examples:
  node world-state.js --world=alpha distance calc lumina-city nexus-station
  node world-state.js --world=alpha distance calc lumina-city the-undercrypt manhattan
  node world-state.js --world=alpha distance list
  node world-state.js --world=alpha distance matrix euclidean`);
    }
    break;
...
Usage:
  ...
  node world-state.js --world=<world> distance <command> Calculate distances
...
Commands:
  ...
  distance calc <from> <to> [method]    Calculate distance (euclidean/manhattan/3d)
  distance list                         List all locations with coordinates
  distance matrix [method]              Show distance matrix`);
